...
grafico2 <- ggplot(data = ds, aes(x = MinTemp, y= MaxTemp, color= RainTomorrow))
grafico2 <- grafico2 + geom_point()
grafico3 <- grafico2 + ggtitle("Weather")
...
mañana_grf <- ggplot(ds, aes(x= Temp9am, y = Pressure9am, shape = RainToday, color= RainToday)) + geom_point()
mañana_grf <- mañana_grf + labs(
  title = "Weather 9am",
  subtitle = "Relation between Preassure and Temperatura at 9am",
  x = "Preassure at 9am",
  y = "Temperature at 9am",
  caption = "Data extracted from WeatherAUS dataset",
  colour= "Rained Today?",
  shape= "Rained Today?"
)
mañana_grf
...
```
Con color se observa mejor que con forma la variable RainToday.
...
iris.grafico1 <- ggplot(df, aes(x= Species, y= Value))
iris.grafico1 <- iris.grafico1 + geom_point()

iris.grafico2 <- ggplot(df, aes(x= Species, y= Value, color = Part))
iris.grafico2 <- iris.grafico2 + geom_point() + facet_grid(cols = vars(Measure))
iris.grafico3 <- iris.grafico2 + geom_point() + facet_grid(rows = vars(Measure))
iris.grafico3; iris.grafico2

iris.grafico4 <- iris.grafico2 + geom_jitter()
iris.grafico4

ggsave('figs/Ejercicio3.pdf') # se almacena el último gráfico generado
```
La disposición en columnas permite mejorar los valores de las variables.
Con geom_jitter() hya mayor dispersión de variables. Este desplazamiento separa los puntos que originalmente estaban superpuestos, permitiéndote ver la densidad de los datos en esa ubicación. No altera la posición central de los datos, solo los dispersa ligeramente.
...
grf1 <- ggplot(ejer4, aes(x= Fecha, y= Ventas, color= Fabricante)) + geom_point()
# El gráfico es válido para evaluar la evolción de las ventas de cada fabricante, podemos diferenciarlos bien.

grf2 <- ggplot(ejer4, aes(x= Fecha, y= Ventas, color= Fabricante)) + geom_point() + geom_line(aes(linetype = Fabricante)) + scale_color_manual(values = c("Microsoft" = "green","Apple" = "red")) + labs(
  title = "Ventas mensuales de diferentes compañías tecnológicas",
)

ggsave('figs/Ejercicio4.pdf') # se almacena el último gráfico generado
```
...
grf1 <- ggplot(Experimento, aes(x= Dosis, y= Altura, color= Complemento, linetype = Complemento, shape= Complemento))
grf1 <- grf1 + geom_point() + geom_line()

grf2 <- grf1 + geom_errorbar(aes(ymin = Altura - Error_Altura, ymax = Altura + Error_Altura, width = 0.1))


grf3 <- ggplot(Experimento, aes(x = Dosis, y = Altura, color = Complemento)) + geom_point(size = 3, shape = 16) + geom_line(linetype = "dashed") + geom_errorbar(aes(ymin = Altura - Error_Altura, ymax = Altura + Error_Altura), width = 0.2) + scale_color_brewer(palette = "Reds") +labs(
    title = "Altura según dosis y tipo de complemento (paleta roja)",
    x = "Dosis",
    y = "Altura (cm)"
  )


ggsave('figs/Ejercicio5.pdf') # se almacena el último gráfico generado
...
# Aquí hacemos unas capas y así al solapar ambas vemos el flow de ambos rios
rios <- ggplot(df_rio, aes(x = time)) + geom_line(aes(y = flow.vat, color = "Río Vat")) + geom_line(aes(y = flow.jok, color = "Río Jok")) + scale_color_manual(name = "Ríos", values = c("Río Vat" = "red", "Río Jok" = "blue")) + labs(
    title = "Caudal ríos Vat y Jok",
    subtitle = "Evolución del caudal a lo largo del tiempo",
    x = "Tiempo",
    y = "Caudal"
  )

rios
ggsave('figs/Ejercicio6.pdf') # se almacena el último gráfico generado
...
grf1 <- ggplot(ds, aes(x=Year, y=MinTemp)) + geom_boxplot()

grf2 <- ggplot(ds, aes(x=Year, y=MaxTemp)) + geom_violin(aes(fill=Year))
grf2 <- grf2 + labs(
  title = "Distribución de temperaturas máximas por año",
  x = "Año",
  y = "Temperatura ºC"
) +   theme(
    legend.position = "none", # Elimina la leyenda
    axis.text.x = element_text(angle = 45, hjust = 1)  # Rota las etiquetas
  )

ggsave('figs/Ejercicio7.pdf') # se almacena el último gráfico generado
```
...
grf1 <- ggplot(datos, aes(x=HEIGHT)) + geom_histogram(fill="blue",color="red",alpha=0.5,breaks=seq(155,195, length.out=21)) + labs(
    title = "Histograma de la variable Altura",
    x = "Altura (cm)",
    y = "Frecuencia"
)
grf1
grf2 <- ggplot(datos, aes(x = HEIGHT)) +
  geom_histogram(
    aes(y = after_stat(density)),
    fill = "blue",
    color = "red",
    alpha = 0.5,
    breaks = seq(155, 195, length.out = 21)
  ) +
...
grf2
grf3 <- ggplot(datos, aes(x = HEIGHT)) +
  geom_histogram(
    aes(y = after_stat(density)),
    fill = "blue",
    color = "red",
  ...
  geom_density(color = "green", size = 1) +
  labs(
    title = "Histograma de la variable Altura por MORT",
    x = "Altura (cm)",
    y = "Frecuencia"
  ) +
  facet_grid(. ~ MORT)   
...
ggsave('figs/Ejercicio8.pdf') # se almacena el último gráfico generado
